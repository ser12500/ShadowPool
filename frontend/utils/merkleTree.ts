...
async function hashLeftRight(left: string, right: string) {
...
export class PoseidonTree {
    ...
    async update(index: number, newLeaf: string, isInsert = false) {
        ...
        await this.traverseAsync(index, async (level: number, currentIndex: number, siblingIndex: number) => {
            const sibling = this.storage.get(PoseidonTree.indexToKey(level, siblingIndex)) || this.zeros[level];
    ...
    traverse(index: number, fn: (level: number, currentIndex: number, siblingIndex: number) => void) {
    ...
    async traverseAsync(index: number, fn: (level: number, currentIndex: number, siblingIndex: number) => Promise<void>) {
        let currentIndex = index;
        for (let level = 0; level < this.levels; level++) {
            const siblingIndex = currentIndex % 2 === 0 ? currentIndex + 1 : currentIndex - 1;
...
// Глобальный экземпляр дерева для использования в приложении
let globalTree: PoseidonTree | null = null;

export async function getGlobalTree(): Promise<PoseidonTree> {
    if (!globalTree) {
        globalTree = await merkleTree([]);
    }
    return globalTree;
}

export async function addLeafToGlobalTree(leaf: string): Promise<PoseidonTree> {
    const tree = await getGlobalTree();
    await tree.insert(leaf);
    return tree;
} 